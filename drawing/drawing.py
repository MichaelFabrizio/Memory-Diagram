...
class Drawing:
    def __init__(self, diagram_width = 16.0, diagram_height = 4.0, show_axes = False):
        self.offset = 0.0
    ...
    def Draw_Vertical_Arrow(self, x0, y0, xf, yf, color, 
                            ...
                            arrow_side_length = 0.2, dashed = False, arrowstyle = '<->', linestyle = '-'):
    ...
    def Draw_Reconnecting_Arrow(self, x_offset, y_offset, stride, cardinality = 'north', height = 1.0, linestyle = '-', arrowstyle = '<->'):
        arrow = arrows.ReconnectingArrow(x_offset, self.offset + y_offset, stride, cardinality, height = height, linestyle = '-', arrowstyle = arrowstyle)
        arrow.Draw(self.ax)

    def Draw_Corner_Arrow(self, x, y, height, width, theta, arrowstyle = '<->'):
        arrow = arrows.CornerArrow(x, self.offset + y, height, width, theta, arrowstyle = arrowstyle)
        arrow.Draw(self.ax)
    ...
    def Draw_Centered_X(self, x_center, y_center, length, color='red', inner_length = 0.2):
        ...
        v3x = x_center
        v3y = y_center + inner_length + self.offset
        v2x = v3x + length_diff
        v2y = v3y + length_diff
        v4x = v3x - length_diff
        v4y = v3y + length_diff
        ...
        v9x = x_center
        v9y = y_center - inner_length + self.offset
        v8x = v9x - length_diff
        v8y = v9y - length_diff
    ...
    def Draw_Line(self, x0, y0, xf, yf, color = 'black'):
        l1 = [(x0, y0), (xf, yf)]
        line1 = self.ax.plot(*zip(*l1), color = color)
    ...
    def Draw_Text(self, text, x0, y0):
        r = self.fig.canvas.get_renderer()
        t = plt.text(x0, self.offset + self.text_offset + y0, str(text), ha='left', va='top', fontweight='bold')
        self.text_offset -= (40.0 / self.fig.dpi) # where 40.0 = (text_height in pixels, 14.0) * 2. + (6 px padding) * 2.

        # IMPORTANT GET TEXT DIMENSIONS IN PIXELS REFERENCE CODE
